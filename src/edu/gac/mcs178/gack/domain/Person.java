...
public class Person {
	...
	public void give(Thing thing) {
     ...
     // otherwise print a message that says “you don’t have the thing”

		if (this!=(thing.getOwner())) {// if the thing’s owner is you (meaning you already own it)
			Utility.displayMessage(thing + " is not owned by " + this);
		} else {
			if (thing.isOwned()) {
				Person user = thing.getOwner();
				user.lose(thing);
				user.haveFit();
			}
			thing.setOwner(this);
